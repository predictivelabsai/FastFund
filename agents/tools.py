...
Each "agent" is a focused capability of the JTC Private Office advisor over the
SFO Hub graph. The orchestrator (a LangGraph tool-calling agent) routes the
...
from agents.context import active_sfo
...
def profile_agent(name: str = "") -> str:
    ...
    asset mix, current JTC services and stated pain points. Pass a name/ref
...
def needs_agent(situation: str) -> str:
    ...
    GAPS plus the JTC service CATEGORIES that address them. Use when the principal
    ...
    out = ["Relevant JTC services for that situation:", svc_ctx or "(none matched)"]
    if bench:
        out.append("\nIndustry context:")
...
def services_agent(query: str = "") -> str:
    """Describe JTC Private Office SERVICES that match a query (e.g. "luxury yacht
    administration", "consolidated reporting", "trust structuring"). Use to explain
    what JTC offers. Returns services with category, tier and an open marker."""
    svcs = store.search_services(query, limit=8) if query else store.list_services(limit=12)
    if not svcs:
        return "No matching JTC service found."
    return "\n".join(
