"""Graph storage backend for FastFund (Neo4j / AuraDB) — the default backend.
...
Design mirrors FastFund's Neo4j store so the same operational story holds:
...
class Neo4jStore(Storage):
    ...
    def _write(self, cypher: str, **params):
        with self._driver.session(database=self._db) as s:
            return s.write_transaction(lambda tx: list(tx.run(cypher, **params)))
