"""FastFund — the FastFund Family Office AI advisor (FastHTML).

A multi-page agentic app mirroring FastFund's structure:
...
from family.agents import orchestrator
from family.agents.context import set_active_sfo
from family.engine import crosssell
from family.web import monitor, coverage as cov, graphdata
...
if os.environ.get("FASTFUND_AUTOSEED", "0") == "1":
    from family.data import synth
    synth.autoseed_if_empty(int(os.environ.get("FASTFUND_SEED_COUNT", "100")))
...
KIND_COLOR = {"cross_sell": "#102a43", "upsell": "#0f766e"}
...
NAV = [
    ("/family/", "🏠 Family advisor", "home"),
    ("/dashboard", "📊 Tax dashboard", "tax-dashboard"),
    ("/family/dashboard", "📈 Family dashboard", "dashboard"),
    ("/family/clients", "👪 Families", "clients"),
    ("/family/opportunities", "🎯 Outreach pipeline", "opportunities"),
    ("/family/calendar", "🗓 Relationship calendar", "calendar"),
    ("/entities", "🏢 Legal entities", "entities"),
    ("/obligations", "✅ Tax obligations", "obligations"),
    ("/calendar", "📅 Filing calendar", "tax-calendar"),
    ("/family/coverage", "🧮 Service coverage", "coverage"),
    ("/family/graph", "🕸 Relationship graph", "graph"),
    ("/family/services", "🗂 Service catalogue", "services"),
    ("/documents", "📚 Tax knowledge", "tax-documents"),
    ("/family/documents", "📎 Family documents", "documents"),
    ("/user-guide", "❓ FastFund guide", "help"),
]
...
.navlink.on{background:#145f59;font-weight:600}
...
.client:hover{background:#145f59;text-decoration:none}.client.active{background:#145f59;border-color:var(--accent)}
.client .meta{font-size:11px;color:#a8c5c2}
...
.shortcut:hover{background:#145f59}.shortcut b{color:var(--accent)}
...
.toolchip{display:inline-block;font-size:11px;background:#e8f3f2;color:var(--navy2);border:1px solid #e0cfe0;
...
.chips{margin:6px 0}.chip{display:inline-block;background:#e8f3f2;color:var(--navy2);border-radius:20px;
...
.cell.held{background:#102a43}.cell.rec{background:#0f766e}.cell.gap{background:#e6f4f1}
...
.kcol.drop{border-color:var(--accent);background:#eef8f6}
...
# Faceted diamond gem in the FastFund brand palette (navy pavilion, accent crown).
...
# Mirrors FastFund: OIDC via Authlib, /auth/google → Google consent → /auth/callback.
# Set GOOGLE_ALLOWED_DOMAINS (comma-separated, e.g. "fastfund.org") to restrict
...
def _google_button():
    return Div(
        ...
        A(NotStr(_GOOGLE_SVG), "Continue with Google", href="/family/auth/google",
...
def kind_badge(kind):
    return Span("Upsell" if kind == "upsell" else "Cross-sell", cls="pill",
                style=f"background:{KIND_COLOR.get(kind, '#102a43')}")
...
def plotly(div_id, data, layout, height=300):
    ...
    base = {"margin": {"t": 24, "r": 12, "b": 40, "l": 48}, "font": {"size": 12},
            ...
            "colorway": ["#102a43", "#0f766e", "#3c8d87", "#b06b00", "#1c7c44", "#a8c5c2"]}
    base.update(layout)
...
def login_form(sess, error: str = ""):
    return Title("Sign in · FastFund"), CSS, Form(
        H2("FastFund"), P("Single-Family Office (SFO) AI Advisor", style="color:#6b7686"),
        ...
        method="post", action="/family/login", cls="form")
...
def login_submit(sess, email: str = "", password: str = ""):
    ...
    return RedirectResponse("/family/login?error=Invalid+credentials", status_code=303)
...
if OAUTH_ENABLED:
    def _redirect_uri(request):
        ...
        return f"{scheme}://{host}/family/auth/callback"
    ...
    async def google_callback(request, sess):
        try:
            ...
            return RedirectResponse("/family/login?error=Google+sign-in+failed", status_code=303)
        info = token.get("userinfo") or {}
        email = (info.get("email") or "").strip().lower()
        if not email or not info.get("email_verified", True):
            return RedirectResponse("/family/login?error=Google+did+not+return+a+verified+email",
                                    status_code=303)
        if (GOOGLE_ALLOWED_DOMAINS and email.split("@")[-1] not in GOOGLE_ALLOWED_DOMAINS
                and not store.get_user_by_email(email)):
            return RedirectResponse("/family/login?error=This+Google+account+is+not+permitted",
                                    status_code=303)
...
def left_pane(sess, active_id=None, ctx="home"):
    return Div(
        ...
        A("+ New conversation", href="/family/", cls="newchat"),
        Div(Div("Navigate", cls="lbl"),
            *[A(lbl, href=href, cls="navlink on" if key == ctx else "navlink")
              for href, lbl, key in NAV],
            A("↪ Sign out", href="/family/logout", cls="navlink"),
            cls="section"),
        cls="pane left")


def Page(sess, *content, title="FastFund", ctx="home"):
...
def mix_bar(asset_mix):
    colors = {"private_equity": "#102a43", "public_equity": "#0f766e",
              "real_estate": "#3c8d87", "luxury": "#b06b00",
              "cash": "#1c7c44", "alternatives": "#a8c5c2"}
    ...
    if with_actions:
        ...
        body.append(Div(
            Button("Draft proposal", cls="btn ghost sm",
                   hx_post=f"/family/rec/{rid}/proposal", hx_target=f"#rec-{rid}", hx_swap="outerHTML"),
            " ",
            Button("Book consultation", cls="btn ghost sm",
                   hx_post=f"/family/rec/{rid}/book", hx_target=f"#rec-{rid}", hx_swap="outerHTML"),
            " ",
            *[Button(lbl, cls="btn ghost sm",
                     hx_post=f"/family/rec/{rid}/status?status={st}", hx_target=f"#rec-{rid}",
                     hx_swap="outerHTML")
    ...
    recs_rendered = ([rec_card(r) for r in recs] if recs else
                     [P("No recommendations yet.", cls="kv"),
                      Button("Generate recommendations", cls="btn",
                             hx_post=f"/family/recommend/{sfo_id}", hx_target="#right", hx_swap="outerHTML")])
    return Div(
        Div(A(sfo["name"], href=f"/family/sfo/{sfo_id}", style="color:var(--navy)"),
            A("Open ↗", href=f"/family/sfo/{sfo_id}", style="font-size:12px"), cls="rhead"),
        Div(Div(H3(sfo["name"]),
...
function mk(html){return html.replace(/\\[service:(\\d+)\\]/g,(m,id)=>` <a href="/family/service/${id}">↗</a>`)
  .replace(/\\[sfo:(\\d+)\\]/g,(m,id)=>` <a href="/family/sfo/${id}">↗</a>`).replace(/\\[rec:(\\d+)\\]/g,'');}
async function sfoSend(){
  ...
  const resp=await fetch('/family/chat__SFO__',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({q})});
...
def sfo_detail(sess, sfo_id: int):
    ...
    import taxstore as tax_store
    entities = [e for e in tax_store.list_entities(limit=1000)
                if e.get("sfo_id") == sfo_id]
    ...
    return Page(sess,
        Div(A("← Advisor", href=f"/family/?sfo={sfo_id}"), style="margin-bottom:8px"),
        H1(sfo["name"]),
        ...
        Div(Div(H2("Asset allocation"), donut, cls="card"),
            Div(H2("Profile"),
                ...
                cls="card"),
            cls="cardrow"),
        H2("Legal entities & tax filings"),
        (Table(Tr(Th("Entity"), Th("Type"), Th("Domicile"), Th("Jurisdictions")),
               *[Tr(Td(A(e["name"], href=f"/entity/{e['id']}")),
                    Td(e.get("type") or "—"), Td(e.get("domicile") or "—"),
                    Td(", ".join(e.get("jurisdictions") or []) or "—"))
                 for e in entities]) if entities else
         P("No legal entities are linked yet. ",
           A("Add one in Tax & Compliance", href="/entities"), ".", cls="kv")),
        H2("Family members"),
        ...
        Div(H2("Recommendations"),
            Button("↻ Regenerate", cls="btn ghost sm", style="float:right",
                   hx_post=f"/family/recommend/{sfo_id}/page", hx_target="#recs", hx_swap="innerHTML"),
            style="overflow:auto"),
        ...
        (Table(Tr(Th("Due"), Th("Action"), Th("Title"), Th("Urgency"), Th("")),
               *[Tr(Td(a.get("due_date") or "—"),
                    ...
                    Td(Button("Done", cls="btn ghost sm",
                              hx_post=f"/family/action/{a['id']}/done", hx_target="closest tr", hx_swap="outerHTML")))
                 for a in actions]) if actions else P("No scheduled actions.", cls="kv")),
        H2("Documents"),
        (Ul(*[Li(A(d["name"], href=f"/family/document/{d['id']}/file"),
        ...
        (Ul(*[Li(A(c["title"] or "Conversation", href=f"/family/?sfo={sfo_id}"),
                 f" · {(c.get('updated_at') or '')[:10]}") for c in convs])
         if convs else P("No conversations yet.", cls="kv")),
        title=f"{sfo['name']} · FastFund", ctx="home")
    ...
    return Page(sess, H1("New family office — onboarding"),
        ...
        Form(
            ...
            Div(Button("Build profile & roadmap", cls="btn", style="margin-top:16px"),
                style="margin-top:8px"),
            method="post", action="/family/onboard"),
        title="Onboarding · FastFund", ctx="clients")
...
async def onboard_submit(sess, request):
    ...
    set_active_sfo(sfo_id)
    crosssell.recommend(sfo_id, persist=True, use_ai=True)
    return RedirectResponse(f"/family/sfo/{sfo_id}", status_code=303)
...
def clients(sess, stage: str = ""):
    ...
    chip = lambda lbl, q, on: A(lbl, href=q, cls="urg",  # noqa: E731
                                style=f"background:{'#102a43' if on else '#9a93a6'};margin-right:6px")
    filters = Div(chip("All", "/family/clients", not stage),
                  *[chip(s.title(), f"/family/clients?stage={s}", stage == s)
                    for s in ("lead", "onboarding", "client")],
                  style="margin:10px 0")
    rows = [Tr(Td(A(B(s["name"]), href=f"/family/?sfo={s['id']}")),
               ...
               Td(", ".join((s.get("current_services") or [])[:4]) or "—",
                  style="color:var(--muted)"),
               Td(A("Open ↗", href=f"/family/sfo/{s['id']}")))
            for s in sfos]
    return Page(sess,
        Div(H1("Client book", style="display:inline-block"),
            A("+ New family office", href="/family/onboard", cls="btn sm", style="float:right;margin-top:6px"),
            style="overflow:auto"),
        ...
        title="Client book · FastFund", ctx="clients")
...
def _kcard(r):
    return Div(
        ...
        Div(A(r["sfo_name"], href=f"/family/sfo/{r['sfo_id']}", draggable="false"), cls="kc-sfo"),
  ...
  document.querySelectorAll('.kcol').forEach(col=>{
    ...
    col.addEventListener('drop',async e=>{
      ...
      if(card && card.dataset.status!==status){
        ...
        kbCounts();
        try{await fetch('/family/rec/'+dragId+'/status?status='+status,{method:'POST'});}catch(e){}
      }
    });
  });
}
...
def opportunities(sess, kind: str = "", category: str = ""):
    ...
    return Page(sess,
        ...
        Div(*cols, cls="kanban"),
        KANBAN_JS,
        title="Pipeline · FastFund", ctx="opportunities")
...
def calendar(sess, urg: str = ""):
    ...
    chips = [A(f"{monitor.URGENCY_LABEL[u]} ({counts.get(u,0)})", href=f"/family/calendar?urg={u}",
    ...
    table = (Table(Tr(Th("Due"), Th("In"), Th("Family"), Th("Action"), Th("Title"), Th("Urgency"), Th("")),
                   *[Tr(Td(a.get("due_date") or "—"),
                        ...
                        Td(A(a.get("sfo_name") or "—", href=f"/family/sfo/{a['sfo_id']}")),
                        Td(ACTION_LABELS.get(a.get("kind"), a.get("kind"))),
                        Td(a.get("title")), Td(urgency_badge(a["urgency"])),
                        Td(Button("Done", cls="btn ghost sm",
                                  hx_post=f"/family/action/{a['id']}/done", hx_target="closest tr", hx_swap="outerHTML")))
    ...
    return Page(sess,
        Div(H1("Pipeline calendar", style="display:inline-block"),
            A("📅 Subscribe / export (iCal)", href="/family/calendar-ics", cls="btn sm",
        ...
        Div(A("All", href="/family/calendar", cls="urg", style="background:#102a43;margin-right:6px"), *chips,
            style="margin:12px 0"),
        table, title="Calendar · FastFund", ctx="calendar")


def _ics(actions) -> str:
    """Build a VCALENDAR of open next-actions (all-day events)."""
    lines = ["BEGIN:VCALENDAR", "VERSION:2.0", "PRODID:-//FastFund//Pipeline//EN",
             "CALSCALE:GREGORIAN", "METHOD:PUBLISH", "X-WR-CALNAME:FastFund pipeline"]
    ...
    for a in actions:
        ...
        lines += ["BEGIN:VEVENT", f"UID:action-{a.get('id')}@fastfund.predictivelabs.ai",
...
def calendar_ics(sess, sfo: int = 0):
    ...
    return Response(content=_ics(actions), media_type="text/calendar",
                    headers={"Content-Disposition": "attachment; filename=fastfund-pipeline.ics"})
...
def coverage_page(sess):
    ...
    for row in m["rows"]:
        ...
        rows.append(Tr(Td(A(row["sfo"]["name"], href=f"/family/sfo/{row['sfo']['id']}"), cls="sfo"), *cells))
...
def graph(sess, mode: str = "book"):
    ...
    mbtn = lambda m, lbl: A(lbl, href=f"/family/graph?mode={m}", cls="btn sm" + ("" if m == mode else " ghost"))  # noqa: E731
    legend = Span(Span("● ", style="color:#0b1f33"), "Family  ",
                  Span("■ ", style="color:#0f766e"), "Service",
...
net.on('click',p=>{if(!p.nodes.length)return;var n=net.body.data.nodes.get(p.nodes[0]);
  if(n.sfoid)location='/family/sfo/'+n.sfoid; else if(n.serviceid)location='/family/service/'+n.serviceid;});
""")
    return (Title("Relationship graph · FastFund"), CSS,
...
def service_detail(sess, service_id: int):
    ...
    if not s:
        return RedirectResponse("/family/services", status_code=303)
    partners = store.list_cross_sells(s["key"])
    clients = cov.service_clients(store, s["key"], limit=200)
    return Page(sess, Div(A("← Catalogue", href="/family/services"), style="margin-bottom:8px"),
        H1(s["name"]),
        P(f"{CATEGORY_LABELS.get(s.get('category'), s.get('category',''))} · {s.get('tier')}",
          style="color:var(--muted)"),
        P(s.get("description")),
        Div(Div(H2("Commonly bundled with"),
                Ul(*[Li(A(p["name"], href=f"/family/service/{p['id']}"), f" · {p.get('weight',0):.0%}")
                     for p in partners] or [Li("—")]), cls="card"),
            Div(H2(f"Held by {len(clients['holders'])} families"),
                Ul(*[Li(A(c["name"], href=f"/family/sfo/{c['id']}")) for c in clients["holders"][:12]]
                   or [Li("—")]),
                H2(f"Recommended to {len(clients['recommended'])}"),
                Ul(*[Li(A(c["name"], href=f"/family/sfo/{c['id']}")) for c in clients["recommended"][:12]]
                   or [Li("—")]), cls="card"),
            cls="cardrow"),
        title=f"{s['name']} · FastFund", ctx="services")
...
def documents(sess):
    ...
    rows = [Tr(Td(A(d["name"], href=f"/family/document/{d['id']}/file")),
               Td(d.get("doc_type", "")), Td(A(d.get("sfo_name") or "—", href=f"/family/sfo/{d['sfo_id']}") if d.get("sfo_id") else "—"),
    ...
    return Page(sess, H1("Documents"),
        ...
        Form(Select(Option("— attach to family —", value=""),
             ...
             cls="filters", method="post", action="/family/upload", enctype="multipart/form-data"),
...
async def upload(sess, request):
    ...
    if sfo_id and text:
        ...
        if applied:
            set_active_sfo(sfo_id)
            crosssell.recommend(sfo_id, persist=True, use_ai=True)
    return RedirectResponse(f"/family/sfo/{sfo_id}" if sfo_id
                            else "/family/documents", status_code=303)
...
HELP_SECTIONS = [
    ...
    ("ug-11-services.png", "Service catalogue",
     "The FastFund Family Office offerings the advisor cross-sells and upsells — open a service "
...
def user_guide_pdf(sess):
    if require(sess):
        return RedirectResponse("/family/login", status_code=303)
    import glob
    # Serve the newest date-stamped guide (docs/fastfund_user_guide_<date>.pdf),
    # falling back to the legacy undated file.
    dated = sorted(glob.glob("docs/fastfund_user_guide_*.pdf"))
    path = dated[-1] if dated else "docs/fastfund_user_guide.pdf"
    if not os.path.exists(path):
        return JSONResponse({"error": "user guide not generated"}, status_code=404)
    return FileResponse(path, media_type="application/pdf",
                        headers={"Content-Disposition": "inline; filename=fastfund-user-guide.pdf"})
    ...
    for i, (t, d) in enumerate(JOURNEY_STEPS, 1):
        cards.append(Div(
            Div(str(i), style="width:30px;height:30px;border-radius:50%;background:var(--navy);"
                ...
                "margin-bottom:10px"),
            B(t, style="color:#0b1f33;font-size:14px;display:block;margin-bottom:5px;line-height:1.2"),
...
def help_page(sess):
    if (r := require(sess)):
        return r
    return RedirectResponse("/user-guide", status_code=303)
    sections = []
    ...
    return Page(sess,
        Div(H1("FastFund — User Guide", style="display:inline-block"),
            Span(A("📑 Slide deck (PDF)", href="/family/user-guide-pdf", cls="btn sm"),
                 " ", A("Technical guide →", href="/family/technical-guide", cls="btn ghost sm"),
                 style="float:right;margin-top:8px"), style="overflow:auto"),
        P("FastFund is an AI relationship-manager for FastFund's Private Office. It "
        ...
        Div(Div(B("Quick start "), "Open a family from the ", B("Client book"),
                ...
                style="background:#eef8f6;border:1px solid var(--line);border-radius:10px;"
        ...
        *sections,
        ...
        P(I("All family-office data is synthetic. No real client data is used."),
          style="color:var(--muted)"),
        title="User Guide · FastFund", ctx="help")
...
_MERMAID = {
  ...
  HOST --> BUILD[Docker build · port 5021] --> RUN[Rolling update · /health] --> LIVE([fastfund.predictivelabs.ai])
...
MERMAID_INIT = Script(
    ...
    "themeVariables:{primaryColor:'#e8f3f2',primaryBorderColor:'#0f766e',"
    "lineColor:'#3c8d87',primaryTextColor:'#48484f'}});",
    type="module")
...
def technical_guide_pdf(sess):
    ...
    return FileResponse("docs/technical_architecture_slides.pdf",
                        ...
                        headers={"Content-Disposition": "inline; filename=fastfund-architecture.pdf"})
...
def technical_guide(sess):
    ...
    return Page(sess, H1("FastFund — Technical guide"),
        Div(A("← Help", href="/family/help"),
            A("📑 Download slide deck (PDF)", href="/family/technical-guide-pdf",
        ...
        P("Full agentic architecture — the LangGraph orchestrator, its six specialist "
          ...
          "Markdown live in ", A("docs/technical_architecture.md", href="https://github.com/predictivelabsai/fastfund/blob/main/docs/technical_architecture.md"),
          ".", style="color:var(--muted)"),
        H2("Stack"),
        ...
        *diagrams,
        MERMAID_INIT,
        title="Technical guide · FastFund", ctx="help")
