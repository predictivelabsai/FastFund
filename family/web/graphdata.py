...
Two modes (analogue of FastFund's ontology provenance/schema):
...
* **schema** — the FastFund service catalogue + the `CROSS_SELLS_TO` graph: how
  services bundle, independent of any client.
"""
from __future__ import annotations

# vis-network groups (FastFund palette).
GROUPS = {
    "sfo": {"shape": "dot", "color": "#0b1f33"},
    "service": {"shape": "box", "color": "#0f766e"},
    "service_held": {"shape": "box", "color": "#102a43"},
}
        ...
        for key in held:
            ...
            if svc:
                edges.append({"from": sid, "to": add_service(svc, True),
                              "color": {"color": "#102a43"}, "title": "holds"})
        for r in store.list_recommendations(sfo_id=s["id"]):
            ...
            if svc and svc["key"] not in held:
                edges.append({"from": sid, "to": add_service(svc, False),
                              "dashes": True, "color": {"color": "#b4cfca"},
