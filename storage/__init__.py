"""Pluggable storage for SFO Hub.

The active backend is chosen by the ``DATA_STORAGE`` environment variable:

* ``neo4j``  (default) — graph backend; the SFO/service/recommendation data is
  naturally graph-shaped (cross-sell edges, holdings, conversation threads).
* ``sqlite`` — relational backend (also reachable as Postgres via ``DB_URL``);
  the zero-infra path for local demos.

Callers use ``get_store()`` (or the ``sfostore`` compat shim) and never import a
