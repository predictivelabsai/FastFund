...
from family.storage.sqlite_store import SqliteStore
...
def _neo4j():
    import socket
    try:
        with socket.create_connection(("127.0.0.1", 7687), timeout=0.25):
            pass
    except OSError as e:
        pytest.skip(f"Neo4j unavailable: {e}")
    try:
        from family.storage.neo4j_store import Neo4jStore
        s = Neo4jStore()
        s.init_db()
        return s
...
def test_conversations(store):
    ...
    cid = store.create_conversation("admin@fastfund.org", sfo_id=sid, title="Intro")
    store.add_message(cid, "user", "hi")
    store.add_message(cid, "assistant", "hello")
    msgs = store.get_messages(cid)
    assert [m["role"] for m in msgs] == ["user", "assistant"]
    assert store.list_conversations(user_email="admin@fastfund.org")[0]["id"] == cid
