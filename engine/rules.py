...
asset mix, pain points, AUM, lifecycle stage) and proposes a JTC service to
...
A rule is a dict:
    {
      ...
      "service": service_key,        # target JTC service
...
RULES = [
    ...
    {"id": "edge_reporting", "when": lambda p: (_pain(p, "reporting", "consolidat", "visibility", "complexity")
     ...
     "rationale": "Reporting complexity across multiple services — JTC Edge gives "
    ...
    {"id": "large_aum_private_office", "when": lambda p: (p.get("aum_usd") or 0) >= 1_000_000_000
     ...
     "rationale": "Billion-dollar-plus AUM — a dedicated JTC Private Office "
